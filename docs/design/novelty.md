...
既存のターンテイキング AI は **音声 1 モダリティで「次の 200ms に喋るか」の二値判定** に集約している（VAP, Smart Turn, MaAI）。本プロジェクト ITM は、(1) **話者交代・相づち・割り込みの 3 種類を同じモデルで同時に**「いつ来るか」を連続予測する **サバイバルハザード形式** に拡張し、(2) **顔映像** を呼吸・口形・視線など 3 経路で late-fusion し、(3) **8M params 級でスマホ・PC で動く軽量実装** にする。視覚統合は Obi & Funakoshi (ICMI 2023) が先行するが彼らは接触型ベルトや 3DCNN 直接回帰を要求しており、**顔のみで動く実装はオープンソースに存在しない**。
