...
- [x] **Phase 2-B v6-α**（2026-05-16）: AMI 17 meetings (8.6h) で再学習 → **Overall 0.677** (baseline 0.586 超え、calibration 改善)、ただし AUC は 0.535
- [x] **Phase 2-B v7**（2026-05-17、negative result）: per-frame BCE への revert を試したが二極化で regression (AUC 0.501)。**Phase 2-B 確定モデル = v6-α**
- [ ] 既存 VAP_BC、VAP_Nod との比較（v2 で扱う）
- [ ] 既存 VAP_BC、VAP_Nod との比較

### Phase 3 — 視覚追加（撤退、2026-05-18）

期間: 当初 約 1 週間 → 実施 1 日で打ち切り

- [x] MediaPipe Face Landmarker による特徴抽出 (52 blendshapes + Euler + mouth open = 56 dim @ 25 fps、17 meetings 全部、335MB)
- [x] late additive fusion (`h = h + α * visual_proj(v)`) で v8 学習
- [x] AMI で fine-tune (v6-α と同設定 + `--use-visual`、CPU 109 min)
- [x] アブレーション: **v8 (AUC 0.412) は v6-α (0.535) から大幅悪化**
- [x] Codex 提案の visual-only probe で原因確認: visual feature 単独 ROC-AUC ≈ 0.510 (random) → **signal なし**
- **撤退**: MediaPipe Face Landmarker は本データ規模では shift discrimination に効かない。視覚 signal は v2 期に rPPG/V-JEPA など別経路で再挑戦
